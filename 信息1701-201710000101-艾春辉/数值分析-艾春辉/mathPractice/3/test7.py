...
#分段线性插值函数
def h(x:[],y:[],h,a):
    ans = grad2(x[0])*(x[1]-a)**3/(6*h)+grad2(x[1])*(a-x[0])**3/(6*h)+(y[0]-grad2(x[0])*(h**2)/6)*(x[1]-a)/h+(y[1]-grad2(x[1])*(h**2)/6)*(a-x[0])/h
    return ans
#实际目标函数
def f(x:float):
    return 1/(1+x**2)
...
def show(n:int):
    ...
    for i in x_predict:
        ...
        # M = []
        #         # M.append()
        y_predict.append(h(x_find,y_find,round(10/n,7),i))
        y_true.append(f(i))
    ...
    # 选中第二个子区域，并绘图
    ax2 = plt.subplot(221)
    plt.scatter(x_predict, y_predict, marker=".", color="red", s=20)
